...
CREATE TABLE group_configs (
  ...
  registration_type VARCHAR(10) NOT NULL DEFAULT 'self',
...
COMMENT ON COLUMN group_configs.registration_type IS '등록 방식: self (직접등록) | preset (사전등록)';
...
COMMENT ON COLUMN event_participants.is_preset IS '사전등록 여부 (true: 관리자가 사전등록, false: 참가자 직접등록)';
...
COMMENT ON TABLE preset_members IS '사전등록 멤버 템플릿 (그룹 설정에 연결)';
COMMENT ON COLUMN preset_members.group_config_id IS '소속 그룹 설정';
COMMENT ON COLUMN preset_members.name IS '이름 (최대 10글자)';
COMMENT ON COLUMN preset_members.department IS '소속 팀 (팀 구분 모드에서 사용)';

-- ============================================================
-- 7. match_results (매칭 결과 - 조 배정)
-- ============================================================
...
CREATE POLICY "anon_select_preset_members" ON preset_members FOR SELECT TO anon USING (true);
...
CREATE POLICY "auth_all_preset_members" ON preset_members FOR ALL TO authenticated USING (true) WITH CHECK (true);
...
CREATE INDEX idx_preset_members_group ON preset_members(group_config_id);
